...
export default async function CategoriesPage() {
  ...
  return <section className="shell py-20 sm:py-28">
    ...
    {categories.length ? <RuledGrid className="mt-12" columns="sm:grid-cols-2 lg:grid-cols-3">
      {categories.map((category, index) => <Link key={category.id} href={`/categories/${category.slug}`} className={`ruled-cell group flex min-h-56 flex-col justify-between ${index === 0 ? "bg-white sm:col-span-2 lg:col-span-1" : ""}`}>
