...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
  ...
  const [products, categories, comparisons, stories, editorialLists, articles] = await Promise.all([
    ...
    prisma.comparison.findMany({ where: { status: "PUBLISHED", productA: { status: "PUBLISHED" }, productB: { status: "PUBLISHED" } }, select: { slug: true, updatedAt: true } }),
  ...
  return [
    ...
    ...comparisons.map((comparison) => ({ url: `${siteUrl}/compare/${comparison.slug}`, lastModified: comparison.updatedAt, changeFrequency: "monthly" as const, priority: 0.6 })),
